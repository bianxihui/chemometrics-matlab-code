...
if Elm_Type~=REGRESSION
    ...
    %%%%%%%%%%%% 得到的j即为类别数，label（1，j）表示
    sorted_target=sort(cat(2,T,TV.T),2);%目标值进行合并
...
%%%%%%%%%% Calculate training & testing classification accuracy
    ...
    for i = 1 : size(T, 2)
        [x, label_index_expected]=max(T(:,i));  %返回T中第i列的最大值X和最大值所在行数
        [x, label_index_actual]=max(Y(:,i));
        if label_index_actual~=label_index_expected  %判断最大值的位置是否处于同一位置，不是则分错。
